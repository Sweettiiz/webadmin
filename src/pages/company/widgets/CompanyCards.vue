...
import CompanyStatusBadge from '../components/CompanyStatusBage.vue'
...
defineEmits<{
  (event: 'edit', project: Company): void
  (event: 'delete', project: Company): void
}>()
// avatar function use for team section to display an image avatar
...
<template>
  <VaInnerLoading
    v-if="companys.length > 0 || loading"
    :loading="loading"
  ...
  >
    ...
    >
      <VaCardContent class="flex flex-col h-full">
        <div class="text-[var(--va-secondary)]">{{ company.company_create_date }}</div>
        <div class="flex flex-col items-center gap-4 grow">
          ...
          <p>
            <span class="text-[var(--va-secondary)]">Update Date: </span>
            <span>{{ company.company_update_date }}</span>
          </p>
